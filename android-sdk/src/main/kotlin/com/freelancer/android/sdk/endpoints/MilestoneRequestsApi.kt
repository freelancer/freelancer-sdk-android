...
interface MilestoneRequestsApi {
    ...
    fun getMilestoneRequestsByProjectId(
            @Query("projects[]") projectId: Long,
    ...
    ): Single<MilestoneRequestResponse>

    @POST("milestone_requests/")
    fun createMilestoneRequest(@Body createMilestoneRequestRequest: CreateMilestoneRequestRequest): Single<MilestoneRequest>

    @PUT("milestone_requests/{milestone_request_id}/")
    fun performMilestoneRequestAction(
            @Path("milestone_request_id") milestoneRequestId: Long,
            @Query("action") action: Milestone.MilestoneAction,
            @Query("reason") reason: String? = null): Single<ApiResponse>
}